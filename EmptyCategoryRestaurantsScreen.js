...
export default function EmptyCategoryRestaurantsScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      const restaurantReceipts = data.filter(
        item => item?.categories?.categorie_name === 'المطاعم',
      );

      setReceipts(restaurantReceipts);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <View style={categoryFoodStyles.container}>
      ...
      <View style={categoryFoodStyles.headerRow}>
        ...
        <Text style={categoryFoodStyles.title}>مطاعم</Text>
      ...
      <View
        style={[
          categoryFoodStyles.summaryCard,
          {
            ...
            backgroundColor: colors.lightBlueCategory,
          },
        ]}
      >
        <View style={categoryFoodStyles.summaryIconBox}>
          <MaterialIcons name="restaurant" size={42} color={colors.blue} />
        </View>

        <View style={categoryFoodStyles.summaryInfo}>
          <Text style={categoryFoodStyles.summaryTitle}>المطاعم</Text>
      ...
      {filteredReceipts.length === 0 ? (
        <View style={categoryFoodStyles.emptyCategoryContent}>
          <MaterialIcons name="restaurant" size={82} color={colors.border} />
