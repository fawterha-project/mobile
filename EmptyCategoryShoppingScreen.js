...
export default function EmptyCategoryShoppingScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      ...
      const shoppingReceipts = data.filter(
        item => item?.categories?.categorie_name === 'التسوق',
      );

      setReceipts(shoppingReceipts);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <View style={categoryFoodStyles.container}>
      ...
      <View style={categoryFoodStyles.headerRow}>
        ...
        <Text style={categoryFoodStyles.title}>التسوق</Text>
      ...
      >
        <View style={categoryFoodStyles.summaryIconBox}>
          <MaterialIcons name="shopping-bag" size={42} color={colors.purple} />
        </View>

        <View style={categoryFoodStyles.summaryInfo}>
          <Text style={categoryFoodStyles.summaryTitle}>التسوق</Text>
      ...
      {filteredReceipts.length === 0 ? (
        <View style={categoryFoodStyles.emptyCategoryContent}>
          <MaterialIcons name="shopping-bag" size={82} color={colors.border} />
